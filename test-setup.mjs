...
// Keep every workspace test process away from the user's real pi-code home.
...
const trackEnvironment = (environment) => new Proxy(environment, {
  set(target, property, value) {
    if (property === "PI_CODE_TEST_HOME") {
      ...
      // runtime code itself reads only PI_CODE_HOME.
      target.PI_CODE_HOME = value;
      if (isDisposableHome(value)) disposableHomes.add(resolve(value));
    }
    return Reflect.set(target, property, value);
  },
  deleteProperty(target, property) {
    if (property === "PI_CODE_TEST_HOME") delete target.PI_CODE_HOME;
    return Reflect.deleteProperty(target, property);
  },
});
process.env = trackEnvironment(process.env);
const testHome = mkdtempSync(join(tmpdir(), "xzy-pi-code-test-home-"));
