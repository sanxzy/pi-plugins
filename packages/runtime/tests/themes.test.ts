...
function mutableBuiltins(): ThemeLibrary {
  return structuredClone({ version: 1, profiles: BUILTIN_THEME_PROFILES });
}
...
test("first use publishes the complete built-in library privately", () => withHome(() => {
  ...
  assert.deepEqual(readdirSync(dirname(homeThemesFile())), [THEMES_FILE_NAME]);
...
test("valid external edits are detected without clearing the process cache", () => withHome(() => {
...
for (const [label, mutate] of [
  ["incomplete colors", (library: ThemeLibrary) => { delete library.profiles[0]!.colors.accent; }],
  ...
  test(`invalid library is backed up and regenerated: ${label}`, () => withHome(() => {
    loadThemeLibrary();
...
test("backup numbering never overwrites invalid or valid recovery evidence", () => withHome(() => {
  loadThemeLibrary();
...
test("publication failure after backup restores the original primary", () => withHome(() => {
  loadThemeLibrary();
...
test("lookup returns a defensive profile and deterministic built-in fallback", () => withHome(() => {
  const library = loadThemeLibrary();
