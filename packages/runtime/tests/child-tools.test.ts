...
const ALL_CHILD_TOOLS = [...ALL_BUILTIN_TOOLS, "web_search", "web_fetch"];
...
test("the default agent enables all seven built-in tools plus the web tools", () => {
  assert.deepEqual(resolveChildTools(DEFAULT_AGENT), ALL_CHILD_TOOLS);
});

test("an explicit non-empty tools list keeps the allowlist and appends the web tools", () => {
  const tools = ["read", "grep"];
  assert.deepEqual(resolveChildTools(discoveredAgent(tools)), [...tools, "web_search", "web_fetch"]);
});
...
test("explicit child tool lists cannot enable goal capabilities but gain the web tools", () => {
  ...
  assert.deepEqual(resolveChildTools(discoveredAgent(tools)), ["read", "web_search", "web_fetch"]);
});
...
test("the default allowlist includes the read-only built-in and web tools", () => {
