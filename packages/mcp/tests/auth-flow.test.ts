...
test("remote auth mutation boundaries emit telemetry and preserve failure propagation", async () => {
  ...
  const authEvents = events.filter((record) => record.operation === MCP_OPERATIONS.AUTH_STORE);
  ...
  assert.equal(errors.filter((record) => record.operation === MCP_OPERATIONS.AUTH_STORE && record.phase === "error").length, 1);
  rmSync(agentDir, { recursive: true, force: true });
});
